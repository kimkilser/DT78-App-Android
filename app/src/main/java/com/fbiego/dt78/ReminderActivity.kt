...
import kotlinx.android.synthetic.main.activity_reminder.sedStart
...
class ReminderActivity : AppCompatActivity() {
    ...
    override fun onResume() {
        ...
        var sed = dbHandler.getSet(0)
        hr24 = !(setPref.getBoolean(SettingsActivity.PREF_12H, false))

        if (sed.isNotEmpty()){
            current.addAll(sed)
        } else {
            val sd = arrayOf(0, 7, 0, 22, 0, 0, 30)
            current.addAll(sd)
        ...
        sedStart.setOnClickListener{
            val picker = TimePickerDialog(this, TimePickerDialog.OnTimeSetListener { _, i, i2 ->
                current[1] = i
                current[2] = i2
                sedStart.text = java.lang.String.format("%02d:%02d", i, i2)
                dbHandler.insertSet(current)
        ...
        sedEnd.setOnClickListener {
            val picker = TimePickerDialog(this, TimePickerDialog.OnTimeSetListener { _, i, i2 ->
                current[3] = i
                current[4] = i2
                sedEnd.text = java.lang.String.format("%02d:%02d", i, i2)
                dbHandler.insertSet(current)
                FG().updateSed(current)
    ...
    private fun alarmClicked(alarm: AlarmData){
        ...
        val switch = alarmLayout.findViewById<Switch>(R.id.enableSwitch)
        val week = alarmLayout.findViewById<LinearLayout>(R.id.week)
        var spinner = alarmLayout.findViewById<Spinner>(R.id.alarmRepeat)
        ...
        time.text = java.lang.String.format("%02d:%02d", hr, min)
        ...
        spinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener{
            ...
            override fun onItemSelected(p0: AdapterView<*>?, p1: View?, p2: Int, p3: Long) {
                if (getString(R.string.custom) == p0?.getItemAtPosition(p2).toString()){
